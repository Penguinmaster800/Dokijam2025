...
func _ready():
	_update_options()
	_randomize_options()
	_spawn_upgrade_1(random_options[1])
	_spawn_upgrade_2(random_options[2])
	_spawn_upgrade_3(random_options[3])
...
func _randomize_options():
	random_options = upgrade_options[randi()%len(upgrade_options)]
